...
public class NurseBlock extends Block {
    public NurseBlock(Properties properties) {
        super(properties);
    }
    ...
    // 这里例如是livingEntity就给该实体加上一个***的药水效果。
    // 其中MobEffectInstance类是实体身上的药水的效果类。作用是给实体药水的效果。
    // MobEffects类定义了各个药水的效果。100代表的是持续时间(20:1)
    @Override
    public void stepOn(Level level, BlockPos blockPos, BlockState blockState, Entity entity) {
        if(entity instanceof LivingEntity livingEntity){
            livingEntity.addEffect(new MobEffectInstance(MobEffects.HEAL,1200));
            livingEntity.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST,1200));
        }
